...
def read_cpl(fname, idx=None, dir_cpl=JOIN(DIR_PROD, 'cpl', 'nc'),
	...
	for t in xrange(idx.size):
		...
		cpl.depolarization[t] = h_cpl.variables['Depolorization'][i][:] #_sic
	...
	if manual_tau:
		...
		for i, fov in enumerate(cpl):
			...
			#_loop over 355 layers, find top and bottom
		#	idx_top, idx_bot = [], []
			idx_top, idx_bot = {}, {} 
			...
			if 1:
				...
				props = dict(facecolor='white', alpha=0.99)
				
				fig, axes = plt.subplots(1, len(wavelengths) + 1)
				...
				for j, wl in enumerate(wavelengths):
					...
					for ibot, itop in zip(idx_bot[wl], idx_top[wl]):
			
						ax.plot([0, 1e-3], [itop, itop], 'r-', linewidth=0.5, zorder=0)
						ax.plot([0, 1e-3], [ibot, ibot], 'g--', linewidth=0.5, zorder=1)

						tmp = x[ibot:itop]
						tmp = tmp[tmp > -1e-8]
						tau = trapz(tmp) * 30
						ax.text(.65e-4, (itop+ibot)/2., '{0:5.3f}'.format(tau),
							size='small', bbox=props)
				...
				axes[-1].plot(fov.depolarization, y, linewidth=0.3)
				axes[-1].set_title('DEPOL')
				...
				pname = 'cad_algo_{0}_f{1:04d}.png'.format(res.group(1),i)
