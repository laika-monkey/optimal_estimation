...
namelist = {
	#_date range to process
	'dtg0'			: '20130824000000',
	'dtg1'			: '20130825000000',
	
	#_lat/lon range to process
	'latlon'		: [-30, 30, -90, 20],
...
##	'surf_temp_src'		: 'ECMWF',	#_GDAS || LBL-ATM || ECMWF || SHIS (n/a)
	...
	'dynamic_CTP'		: False, # ['NAAPS','SHIS'], False,
	'dynamic_ATP'		: ['NAAPS'], # ['SHIS'], False,
...
##	############################################################################
	...
	'sensor' : 'AIRS',		# define the sensor used for the retrieval.
...
def real_case_sbatch(fidx=None, rerun=True, dir_plot='.', bias_correct=False,
...
def lblrtm_range_airscal(dtg0='19900101000000', dtg1='20820223000000',
	latlon=None,
	dir_colloc=os.path.join(os.environ['WORK'], 'colloc'), 
	dir_airs=os.path.join(os.environ['PRODUCTS'], 'sips/airs'), 
	pname='/home/wsessions/data/updated_airscal.pk', **kwargs):
	'''
	Process LBL-RTM profiles within a given range based upon airscal files
	'''
	from pyhdf.SD import SD, SDC
	from qsubmissions import lblrtm_sbatch
	from libtools import epoch2dtg as e2d
	from libtools import dtg2epoch as d2e 
	from numpy import arange, diff
	from oe_utils import read_airs, get_collocation_indices
	from oe_utils import get_collocation_files
	from glob import glob

	#_get list of collocation files from pickle made on kepler
	files_col = glob(dir_colloc + '/colloc*')
	files_col.sort()

	#_convert time to epoch
	...
	for fname in files_col:
		...
		#_create full pathname for input file
		fname = os.path.join(dir_colloc, fname)

		#_read in indices
		hdf = SD(fname, SDC.READ)
	##	idx_x = hdf.select('Master_Index_1')[:] - 1
	##	idx_a = hdf.select('Master_Index_2')[:] - 1
		idx_x, idx_a, idx_c = get_collocation_indices(fname, **kwargs)

		#_pull out name for airs file
		fairs = get_collocation_files(fname)['AIRS']
	##	fairs = hdf.attributes()['fname_AIRS'] #_WRS
		airs, wave = read_airs(fairs, idx_x, idx_a, **kwargs)

		#_if any within range, pass to lblrtm_sbatch 
		fidx = (airs.AIRS_epoch >= e0) * (airs.AIRS_epoch < e1)
		
		#_if lat/lon is not none, also limit
		...
		#_nothing within time frame
		if fidx.sum() == 0:
			continue	
		print 'MATCHES', fidx.sum()

		#_convert to list of numbers
		fidx = arange(fidx.size)[fidx]
		fidx = [int(i) for i in fidx]
	
		if True:
			tmp = fname.split('/')
			with open('matches_lblrtm.{0}.txt'.format(tmp[-1]), 'w') as f:
				for i in fidx:
					e = airs.AIRS_epoch[i]
					f.write('{0} {1:f}\n'.format(e2d(e,full=True), e)) 

		#_launch associated fields of view
		lblrtm_sbatch(fname, fidx=fidx, **kwargs) 


def lbldis_range_airscal(dtg0='19900101000000', dtg1='20820223000000',
	files_col=None, pname='/home/wsessions/data/updated_airscal.pk', **kwargs):
	'''
	Process LBL-RTM profiles within a given range based upon airscal files

	Purpose of this level is to limit in time/space which indices are run
	'''
	from pyhdf.SD import SD, SDC
	from qsubmissions import lbldis_sbatch
	from pickle import load
	from libtools import epoch2dtg as e2d
	from libtools import dtg2epoch as d2e 
	from numpy import arange
	from oe_utils import read_airs, get_collocation_indices, precheck
	from glob import glob
	...
	#_get list of files
	dir_colloc = kwargs.get('dir_colloc',
		os.path.join(os.environ['WORK'], 'colloc'))
	latlon = kwargs.get('latlon', [-90,90,-180,180]) 

	#_get names of all the collocation files
	if files_col is None:
		files_col, pre_create = precheck(e0=e0, e1=e1, **kwargs)
	else:
		pre_create = False
	files_col.sort()

	#_odds and ends
	if kwargs.get('surf_temp') == -1:
	...
	kwargs.update({
			'lbldis_output'	: 'lbldis_output.{0}.final.cdf'.format(out_label),
			'lbldis_clear'	: 'lbldis_output.{0}.clear.cdf'.format(out_label),
			'posterior_dump' : 'posterior.{0}.pk'.format(out_label) })
	...
	#	idx_x = hdf.select('Master_Index_1')[:] - 1
	#	idx_a = hdf.select('Master_Index_2')[:] - 1
		idx_x, idx_a, idx_c = get_collocation_indices(fname)

		#_pull out name for airs file
		fairs = hdf.attributes()['fname_AIRS'] #_WRS

		#_read in associated airs fields
		airs, wave = read_airs(fairs, idx_x, idx_a, **kwargs)
		...
		if False:
			tmp = fname.split('/')
			with open('matches_lbldis.{0}.txt'.format(tmp[-1]), 'w') as f:
				for i in fidx:
					e = airs.AIRS_epoch[i]
					f.write('{0} {1:f}\n'.format(e2d(e,full=True), e)) 

		#_launch associated fields of view
		lbldis_sbatch(fname, fidx=fidx, **kwargs) 
...
def plot_jeffz_qsub(dtg=None, fidx=None, out_label='', dz_cpl=25, smooth=True,
	...
	plot_naaps=False, 
	plot_cmask=True,
	gradient_kibosh=True,	#_leave this off.  removes arb spikes.
	**kwargs):
	...
	if plot_cmask: 
		...
		if False:
			...
			cloud_mask[1] = mask_shi
			cloud_mask[2] = mask_btd
			ax3.text(xloc, 0.5,'CPL > {0:4.2f}'.format(cpl_thresh), **props)  
			ax3.text(xloc, 1.5,'SHIS > {0:4.2f}'.format(shis_thresh), **props) 
			ax3.text(xloc, 2.5,'${{\delta}}B_{{\\nu}}T$ > {0:3.1f}K'.format(
				bt_thresh), **props)  
	...
	if plot_aod:
		...
		##	ax0.scatter(y, x, color=color[n], label=out_label, zorder=0,
			ax0.scatter(y, x, color=color[n], label='UW-SSEC/S-HIS', zorder=0,
				marker='x', s=2)
...
def plot_jeffs_qsub(dtg=None, fidx=None, out_label='', dz_cpl=25, smooth=True,
	...
	#	labs.append('{0}={1}'.format(letters[n], '_'.join(out_label.split('_')[2:])))
		...
		#_build label box
		props = dict(boxstyle='round', facecolor='white', alpha=.9)
		ax0.text(4, 4.5, lab, color='k', verticalalignment='top',
...
##def check_missing_airscal(dir_colloc=os.path.join(os.environ['WORK'], 'colloc'),
##	**kwargs):
##	for file_col in files_col:
##		check_missing(file_col, **kwargs)


def check_missing(dir_lblrtm_fmt=None, out_label='test', experiment='hs3',
	dtg0=0, dtg1=0, **kwargs):
	import re
	from oe_utils import read_airs, get_collocation_indices, precheck #_WRS
	from oe_utils import get_collocation_files
	from libtools import epoch2dtg as e2d
	from libtools import dtg2epoch as d2e
	from pyhdf.SD import SD, SDC
	...
	for file_col in files_col:
		...
		#_get name of rads file
		hdf = SD(file_col, SDC.READ)
	
		#_get indices for collocated data
		idx_x, idx_a, idx_c = get_collocation_indices(file_col)
	
		#_read in times to use in output names 
		fname = get_collocation_files(file_col)['AIRS']
		airs, w = read_airs(fname, idx_x, idx_a, **kwargs)
		epoch = airs.AIRS_epoch[:] 
		epoch = epoch[(epoch >= e0) * (epoch < e1)]
		dtgs = e2d(epoch, full=True)

		#_get out coords
		lats = airs.AIRS_latitude[:] #_WRS 
		lons = airs.AIRS_longitude[:] #_WRS 
		...
		#_if lat/lon is not none, also limit
		lat0, lat1, lon0, lon1 = latlon
		lidx = (airs.AIRS_latitude >= lat0) * (airs.AIRS_latitude < lat1)#_WRS
		nidx = (airs.AIRS_longitude >= lon0) * (airs.AIRS_longitude < lon1)#_WRS
		fidx = fidx * lidx * nidx
		...
		missing_count = 0
		missing_count_rtm = 0
		mname = fname.replace('AIRS', 'MISSING_AIRS').split('/')[-1]
		with open(mname, 'w') as f:
			for i, dtg in enumerate(dtgs): #fidx:
	
				#_build file path
				dir_lblrtm = dir_lblrtm_fmt.format(experiment, dtg)
				lbldis = os.path.join(dir_lblrtm, lbldis_output)

				#_check if lblrtm folder is missing
				if not os.path.exists(os.path.join(dir_lblrtm, 'ODdeflt_001')):
					missing_count_rtm += 1
		...
		#_report
		dbg('LBLDIS - {0} missing {1:>6d} / {2:>6d}'.format(
			file_col.split('/')[-1], missing_count, total))
		dbg('LBLRTM - {0} missing {1:>6d} / {2:>6d}'.format(
			file_col.split('/')[-1], missing_count_rtm, total_rtm))
		dbg('\n')	
...
if __name__ == '__main__':
	...
	#_real_#####################################################################
	############################################################################
	#_LBL-RTM, SATELLITE LOOP (must be run before retrieval)
	if 0:
		#_run LBL-RTM within time range	
		lblrtm_range_airscal(**namelist)

	#_OPTIMAL ESTIMATION, SATELLITE LOOP
	if 0:
		lbldis_range_airscal(**namelist)
