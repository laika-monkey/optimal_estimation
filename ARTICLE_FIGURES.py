...
#	'dir_out'		: os.path.join(DIR_PROD, experiment.lower()), 
	...
	'dir_lblrtm_fmt'	: '/data/wsessions/LBL-RTM/test_fov',
    'dir_lblrtm_fmt':os.path.join(DIR_PROD,'LBL-RTM_hs3','{4}/{3}_{0}.{1}_fov{2:06d}'),
...
def read_all_outlabels(comparison='BOTH', **kwargs):
	from hs3_utils import read_retrieved_campaign
	...
	for i, out_label in enumerate(kwargs.get('out_labels')):
		if i > 0:
			modis = False
			cpl = False
			#-only read once

		kwargs.update({'out_label' : out_label, 'modis' : modis, 'cpl' : cpl })
...
def write_all_shit(data, fname=os.path.join(os.environ['WORK'], 'hs3_2013.nc')):
	''' takes giant recarray and dumps it to a netcdf file '''
	from netCDF4 import Dataset
	from numpy import vstack, array, arange

	#_init output file handle
	os.unlink(fname)	
	cdf = Dataset(fname, 'w')

	#_labels of output (all versions and cpl+modis)
	label_order = list(set(data.out_label))
	label_order.sort()

	#_get size of dimensions
	epoch_ref = array(list(set(data.epoch)))
	epoch_ref.sort()
	ntime = epoch_ref.size 
	nlabel = len(label_order)

	#_dimensions will just be time and labels
	cdf.createDimension('time', ntime)
	cdf.createDimension('label', nlabel)

	#_create AOD variable
	vdf_aod = cdf.createVariable('aod', 'f4', ('time', 'label'), fill_value=-9999)
	vdf_aod.labels = ','.join(label_order)
	vdf_cod = cdf.createVariable('cod', 'f4', ('time', 'label'), fill_value=-9999)
	vdf_cod.labels = ','.join(label_order)
	vdf_cmask = cdf.createVariable('cmask', 'f4', ('time', 'label'), fill_value=-9999)

	all_time = cdf.createVariable('epochNL', 'f8', ('time','label'))

	vdf_aod[:] = -9999.
	vdf_cod[:] = -9999.

	#_loop over labels and put into file
	for i, label in enumerate(label_order):
		tmp = data[data.out_label == label]

		sensor = list(set(tmp.sensor))[0]

		#_if S-HIS, add aerosols togethe
		if sensor == 'S-HIS':	
			kaolinite = tmp[tmp.ssp == 'mie_kaolinite']
			gypsum = tmp[tmp.ssp == 'mie_gypsum']
			wat = tmp[tmp.ssp == 'mie_wat']
			
			#_put in order
			kaolinite = kaolinite[kaolinite.epoch.argsort()]
			gypsum = gypsum[gypsum.epoch.argsort()]
			wat = wat[wat.epoch.argsort()]

			if max(kaolinite.epoch - gypsum.epoch) > 0:
				raise RuntimeError, '...'

			for ti, t in enumerate(wat.epoch):
				tidx = epoch_ref == t

				if tidx.sum() == 1:
					tidx = arange(tidx.size)[tidx]	
					vdf_cod[tidx, i] = wat.tau[ti]	

			for ti, t in enumerate(kaolinite.epoch):
				tidx = epoch_ref == t

				if tidx.sum() == 1:
					tidx = arange(tidx.size)[tidx]	
					vdf_aod[tidx, i] = kaolinite.tau[ti] + gypsum.tau[ti]
					vdf_cmask[tidx, i] = kaolinite.cmask[ti] 

		elif sensor == 'CPL':
			cloud = tmp[tmp.ssp == 'cloud']	
			cloud = cloud[cloud.epoch.argsort()]
			aerosol = tmp[tmp.ssp == 'aerosol']	
			aerosol = aerosol[aerosol.epoch.argsort()]
			
			for ti, t in enumerate(aerosol.epoch):
				tidx = epoch_ref == t

				if tidx.sum() == 1:
					tidx = arange(tidx.size)[tidx]
					vdf_aod[tidx, i] = aerosol.tau[ti]
					vdf_cod[tidx, i] = cloud.tau[ti]

		elif sensor == 'MODIS':
			aerosol = tmp[tmp.sensor == 'MODIS']
	...
	dbg('{0} written.'.format(fname))


def fig_CAMPAIGN2(comparison='CPL', **kwargs):
	data = read_all_outlabels(**kwargs)
	fig_CAMPAIGN3(data, **kwargs)
...
def fuck(**kwargs):
	...
	for c in ['CPL', 'MODIS']:
		for f in [True, False]:
			fig_CAMPAIGN4(comparison=c, delta_bt_mask=f, **kwargs)


def fig_CAMPAIGN6(fname=os.path.join(os.environ['WORK'], 'hs3_2013.nc'),
	bins=[[0,0.15], [0.15, 0.3], [0.3, 0.45], [0.45, 0.6], [0.6, 0.8], [0.8,1.],
		[1, 1.25], [1.25, 1.5], [1.5,1.75], [1.75,2.0], [2,3.]],
	residuals=None, dir_plot='.', out_label='.', **kwargs):
	'''
	FULL CAMPAIGN, scatterplot density thing 
	'''
	...
	cdf = Dataset(fname, 'r')
	cdf_aod = cdf.variables['aod']
	cdf_cod = cdf.variables['cod']
	cdf_mask = cdf.variables['cmask']

	bins = linspace(0,3,21)
	...
	fig = plt.figure()
	fig.suptitle('Optical Depth Comparisons for HS3 2013' , size='small')
	...
	labels = cdf_aod.labels.split(',')
	print labels
	idx_modis = labels.index('MODIS')
	idx_cpl = labels.index('CPL')

	print labels
	out_label = 'NVACLIMO1_tau-ref_M2_DV27_CLD_CTP-SHIS_ATP-NAAPS_3KM'

	combos = ((False, 'CPL'), (True, 'CPL'), (False, 'MODIS'), (True, 'MODIS'))
	for i, (delta_bt_mask, comparison) in enumerate(combos):
		print out_label, i, 'TESTING', cdf_mask[:].shape
 
		idx_shis = labels.index(out_label)

		mask = cdf_mask[:, idx_shis]
		if not delta_bt_mask:
			mask[:] = 0

		shis = cdf_aod[mask == 0, idx_shis]	
		modis = cdf_aod[mask == 0, idx_modis]
		cpl = cdf_aod[mask == 0, idx_cpl] 

		#_shis/cpl/modis recarrays should have same size and order
		if comparison == 'MODIS':
			idx = (modis > -1e-5) * (modis <= 4.) * (shis < 4.)
			yy = modis[idx]
				
		elif comparison == 'CPL':
			idx = (cpl > -1e-5) * (cpl <= 4.) * (shis < 4.)
			yy = cpl[idx]
		...
		axis.hexbin(xx, yy, cmap=YOR, mincnt=1, bins='log')
	#	axis.scatter(xx, yy) 
	##	axis.plot([0, 0], [4, 4], 'k--', linewidth=0.2) 
		axis.set_xlim(0, 4) 
		axis.set_ylim(0, 4)
		top_lab = '{0} SHISoe vs {1} (n={2:d})'.format('Cloud Masked,'*delta_bt_mask, comparison, idx.sum())
		axis.set_title(top_lab, size='xx-small')

		if (i % 2) == 0:
			axis.set_ylabel(comparison, size='xx-small')
		if (i / 2) == 1:
			axis.set_xlabel('SHISoe', size='xx-small')	
	
		#_shrink	
		shrink_ticks(axis)

	#_name file and save
	str_mask = '.mask' * delta_bt_mask
	pname = 'camp6.shisoe-cpl-modis.{0}{1}.png'.format('both', str_mask)	
	pname = os.path.join(dir_plot, pname)
	plt.savefig(pname)
	dbg(pname)
...
def fig_CAMPAIGN5(fname=os.path.join(os.environ['WORK'], 'hs3_2013.nc'),
	bins=[[0,0.15], [0.15, 0.3], [0.3, 0.45], [0.45, 0.6], [0.6, 0.8], [0.8,1.],
		[1, 1.25], [1.25, 1.5], [1.5,1.75], [1.75,2.0], [2,3.]],
	residuals=None, dir_plot='.', out_label='.', **kwargs):
	'''
	FULL CAMPAIGN, but show mask vs modis vs cpl
	'''
	...
	np = 4
	fig, ax = plt.subplots(np)
	fig.suptitle('Residual Optical Depths for HS3 2013' , size='small')
	...
	for i, (delta_bt_mask, comparison) in enumerate(combos):
		...
		for b, (bmin, bmax) in enumerate(bins):
			...
			#_shis/cpl/modis recarrays should have same size and order
			if comparison == 'MODIS':
				idx = (modis >= bmin) * (modis < bmax)
				y[b] = (shis[idx] - modis[idx]) / modis[idx] 
				
			elif comparison == 'CPL':
				idx = (cpl >= bmin) * (cpl < bmax)
				y[b] = (shis[idx] - cpl[idx]) / cpl[idx]
		...
		#_make box plot
		axis.plot([0, len(bins)+2], [0, 0], 'k--', linewidth=0.2) 
		axis.boxplot(y, positions=x, widths=0.1)
		axis.set_xlim(0, bins[-1][-1])
		axis.set_ylim(-2, 8)
	#	axis.text(1.5, 6.5, out_label, size='xx-small')
		top_lab = '{0} (SHISoe - {1}) / {1}'.format('Cloud Masked,'*delta_bt_mask, comparison)
		axis.text(2., 6.5, top_lab, size='xx-small')
		
		if i == 3: 
			axis.set_xlabel('Optical Depth Bin Midpoints ({0})'.format(
			comparison), size='xx-small')
	
		else:
			axis.xaxis.set_visible(False)

		axis.set_ylabel('Fractional Residual', size='xx-small')
		...
		if i in [0,2]:
			axis_top = axis.twiny()
			axis_top.set_xlim(0, bins[-1][-1])
			axis_top.set_xticks(x)
			axis_top.set_xticklabels(n, rotation=0)
		#	axis_top.set_xlabel('Number of Retrievals in Bin', size='xx-small')
			shrink_ticks(axis_top)

	#_name file and save
	str_mask = '.mask' * delta_bt_mask
	pname = 'camp5.shisoe-cpl-modis.{0}{1}.png'.format(comparison, str_mask)	
	pname = os.path.join(dir_plot, pname)
	plt.savefig(pname)
	dbg(pname)


def fig_CAMPAIGN4(fname=os.path.join(os.environ['WORK'], 'hs3_2013.nc'),
	out_labels=['NVACLIMO1_tau-ref_M2_DV26_CLD_CTP-NAAPS_3KM'],
	bins=[[0,0.15], [0.15, 0.3], [0.3, 0.45], [0.45, 0.6], [0.6, 0.8], [0.8,1.],
		[1, 1.25], [1.25, 1.5], [1.5,1.75], [1.75,2.0], [2,3.]],
	residuals=None, dir_plot='.', out_label='.', comparison='CPL',
	delta_bt_mask=False, **kwargs):
	'''
	fname	str,	path to output file of optimal estimation
	...
	np = len(out_labels)
	fig, ax = plt.subplots(np)
	fig.suptitle('{1}Residual Optical Depths when Aerosol Layer (S-HIS - {0}) / {0}'.format(comparison,'Cloud Masked '
			*delta_bt_mask), size='small')
	...
	for i, out_label in enumerate(out_labels):
		...
		if i == len(out_labels) - 1: 
			axis.set_xlabel('Optical Depth Bin Midpoints ({0})'.format(
			comparison), size='xx-small')
		axis.set_ylabel('Fractional Residuals', size='xx-small')
		...
		if i == 0:
			...
			axis_top.set_xlim(0, bins[-1][-1])
			axis_top.set_xticks(x)
			axis_top.set_xticklabels(n, rotation=-90)
		#	axis_top.set_xlabel('Number of Retrievals in Bin', size='xx-small')
			shrink_ticks(axis_top)
...
#	ax.text(-0.9, 200, 'n={0}'.format(nstr), fontsize=6)
	...
	#_name file and save
	str_mask = '.mask' * delta_bt_mask
	pname = 'histogram.shisoe-cpl-modis.{0}{1}.png'.format(comparison, str_mask)	
	pname = os.path.join(dir_plot, pname)
	plt.savefig(pname)
	dbg(pname)
...
def fig_CAMPAIGN3(data_all=None,
	out_labels=['NVACLIMO1_tau-ref_M2_DV26_CLD_CTP-NAAPS_3KM'],
	bins=[[0,0.15], [0.15, 0.3], [0.3, 0.45], [0.45, 0.6], [0.6, 0.8], [0.8,1.],
		[1, 1.25], [1.25, 1.5], [1.5,1.75], [1.75,2.0], [2,3.]],
	...
	ssp_aerosol_list = ['mie_quartz', 'mie_kaolinite', 'mie_gypsum']
	ssp_cloud_list = ['mie_wat', 'mie_ice']

	#_read all AOD data into recarray for out_labels
	for i, out_label in enumerate(out_labels):
	
		data = data_all[data_all.out_label == out_label]
		if data.size == 0:
			print 'NO DATA FOR', out_label
			continue
	
		#_list of the number of obs in each bin
		n = []	

	##	data = read_retrieved_campaign(modis=modis, cpl=cpl, out_label=out_label,
	##		**kwargs)
		shis = data[data.sensor == 'S-HIS']
		modis = data_all[data_all.sensor == 'MODIS']
		cpl = data_all[data_all.sensor == 'CPL']
		...
		for b, (bmin, bmax) in enumerate(bins):
			...
			#_shis/cpl/modis recarrays should have same size and order
			if comparison == 'MODIS':
				idx = (modis.tau >= bmin) * (modis.tau < bmax)
				idx = (modis.tau > 0) * idx
				y[b] = []
				
				#_loop over modis times and get difference
				for m in modis[idx]:
					#-pull out time to match with SHIS
					epoch = m.epoch		
					shis_tmp = shis[shis.epoch == epoch]
	
					#_all all the different dust habits together	
					shis_tau = 0.
					for l in shis_tmp:
						if l.ssp in ssp_aerosol_list: 
							shis_tau += l.tau
	
					y[b].append((shis_tau - m.tau) / m.tau)

			elif comparison == 'CPL':
				idx = (cpl.tau >= bmin) * (cpl.tau < bmax)
				idx = (cpl.tau > 0) * idx
				print "TESTING", idx.sum(), data.size, cpl.size, modis.size, shis.size
				y[b] = []

				for c in cpl[idx]:
					#_pull out time to match with shis
					epoch = c.epoch
					shis_tmp = shis[shis.epoch == epoch]

					#_add all the different dust habits
					shis_tau = 0.
					for l in shis_tmp:
						if l.ssp in ssp_aerosol_list:
							shis_tau += l.tau
	
					y[b].append((shis.tau - c.tau) / c.tau)
