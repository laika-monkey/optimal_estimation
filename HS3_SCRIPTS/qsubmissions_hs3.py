...
def run_main(n=1):
##	lblrtm(nfile=n)		#_general lblrtm testing vs. AIRS
##	lbldis(nscene=n)	# general lbldis testing vs. AIRS

	#_just two popular flight dates	
	dtgs = ['20130824212546', '20130825022556']
	dtg = dtgs[1]

	#_lblrtm_hs3_lim only needs file_flight, lbldis_hs3_lim requires both
	FLIGHTS = imp.load_source('', os.path.join(DIR_HS3, 'FLIGHTS')).FLIGHTS
	opts = FLIGHTS[dtg]
	opts.update({
		'file_flight'	: opts['flight_fmt'].format(dtg),
		'file_kw'		: 'mw_kwarg_lbldis.pk' })

	#_CHOOSE
	lblrtm_hs3_lim(**opts)
##	lbldis_hs3_lim(**opts)

##	files_kw = ['mw_kwarg_lbldis.pk', 'full_kwarg_lbldis.pk']
##	for dtg, opts in FLIGHTS.iteritems():
##		opts.update({'file_flight' : opts['flight_fmt'].format(dtg)})
##		lblrtm_hs3(**opts)
##
##		#_loop over full and microwindowed runs
##		for file_kw in files_kw:
##			opts.update({'file_kw' : file_kw})
##			lbldis_hs3_lim(**opts)
...
if __name__ == '__main__': sys.exit(run_main(n=n))
